...
namespace sc {
	namespace Adobe {
		void SCWriter::Init(Context& context) {
			...
			if (context.config.output.empty()) {
				throw exception("Failed to get output path");
			}

			if (context.config.exportToExternalPath.empty()) {
				throw exception("Failed to get external file path");
			}
		};

		pSharedMovieclipWriter SCWriter::AddMovieclip() {
			SCMovieclipWriter* builder = new SCMovieclipWriter(this);

			return std::make_shared<SCMovieclipWriter>(builder);
		}

		pSharedShapeWriter SCWriter::AddShape() {
			SCShapeWriter* builder = new SCShapeWriter(this);

			return std::make_shared<SCShapeWriter>(builder);
		}
		...
		void SCWriter::Finalize() {
			...
			if (m_context->progressBar->window->aboutToExit) {
				...
				swf.save(output, m_context->config.compression);
			}
		}
	}
}