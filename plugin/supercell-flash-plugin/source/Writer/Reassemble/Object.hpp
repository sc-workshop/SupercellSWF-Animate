...
namespace sc::flash
{
	...
	static void get_object_references(SupercellSWF& swf, ReferenceMap& object_references, uint16_t id)
	{
		...
		std::function<void(uint16_t)> process_display_object = [&swf, &add_reference, &process_display_object](uint16_t id)
			{
				add_reference(id);
	...
	{
		swf.movieclips.erase(
			std::remove_if(swf.movieclips.begin(), swf.movieclips.end(), condition),
			swf.movieclips.end()
		);

		swf.shapes.erase(
			std::remove_if(swf.shapes.begin(), swf.shapes.end(), condition),
			swf.shapes.end()
		);

		swf.movieclip_modifiers.erase(
			std::remove_if(swf.movieclip_modifiers.begin(), swf.movieclip_modifiers.end(), condition),
			swf.movieclip_modifiers.end()
		);

		swf.textfields.erase(
			std::remove_if(swf.textfields.begin(), swf.textfields.end(), condition),
			swf.textfields.end()
		);
	}
