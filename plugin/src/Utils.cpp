...
namespace sc {
    namespace Adobe
    {
        fs::path Utils::GetPath(const std::string in) {
#ifdef _WINDOWS
            std::wstring_convert<std::codecvt_utf8_utf16<char16_t>, char16_t> convertCtx;
            return fs::path(convertCtx.from_bytes(in));
#else
            return fs::path(in);
#endif // _WINDOWS
        ...
        void Utils::GetLanguageCode(std::string& langCode, FCM::PIFCMCallback pCallback)
        {
