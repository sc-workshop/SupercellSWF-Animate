...
namespace sc {
	...
	namespace Adobe {
		...
		void FrameBuilder::DeclareFrameElements(SymbolContext& symbol, FCM::FCMListPtr frameElements, std::optional<MATRIX2D> base_transform, bool reverse) {
			PluginContext& context = PluginContext::Instance();
		...
		void FrameBuilder::DeclareFrameElement(
			...
			std::optional<MATRIX2D> base_transform
		)
		{
			PluginContext& context = PluginContext::Instance();
