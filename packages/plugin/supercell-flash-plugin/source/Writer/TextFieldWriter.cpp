...
namespace sc::Adobe {
    ...
    void SCTextFieldWriter::AddParagraph(const TextParagraph& paragraph) {
        ...
        if (m_has_paragraph) {
            context.Trace("TextField in symbol \"%s\" frame %d, has more than one Paragraph. Skip...",
        ...
        // Default value
        if (paragraph.style.lineSpacing == 2) {
        ...
        if (paragraph.textRuns.size() > 1) {
            context.Trace("TextField in symbol \"%s\" frame %d, has more than one TextRun. Using the first text run...",
                          Locale::ToUtf8(m_symbol.name).c_str(),
                          m_symbol.current_frame_index + 1);
        }
    ...
    bool SCTextFieldWriter::Finalize(ResourceReference reference, bool /*required*/, bool new_symbol) {
        if (new_symbol) {
