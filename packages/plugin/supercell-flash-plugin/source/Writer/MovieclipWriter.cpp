...
namespace sc { namespace Adobe {
    ...
    uint16_t SCMovieclipWriter::GetInstanceIndex(uint16_t elementsCount,
        ...
        for (uint32_t i = 0; elementsCount > i; i++) {
            ...
            if (instance.id == id && instance.name.string() == name && instance.blend_mode == blending) {
        ...
        for (uint16_t i = 0; instancesCount > i; i++) {
            ...
            if (instance.id == id && instance.name.string() == name &&
    ...
    void SCMovieclipWriter::AddFrameElement(ResourceReference resource,
        ...
        if (color) {
            ...
            transformColor.alpha =
                (uint8_t) std::clamp((int) ((color->matrix[3][3] * 255) + color->matrix[3][4]), 0, 255);
    ...
    std::size_t SCMovieclipWriter::GenerateHash() const {
        ...
        code.update((const uint8_t*) m_object.frame_elements.data(),
    ...
    bool SCMovieclipWriter::Finalize(ResourceReference reference, bool required, bool new_symbol) {
        ...
        if (m_symbol.linkage_name.empty()) {
            ...
            m_writer.swf.CreateExportName(flash::SWFString(m_symbol.linkage_name), reference.GetId());
