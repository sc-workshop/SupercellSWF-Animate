...
namespace sc::Adobe {
    ...
    AtlasWriter::AtlasWriter(SCWriter& writer) :
        m_writer(writer) {
    }

    void AtlasWriter::ProcessDrawCommand(flash::ShapeDrawBitmapCommand& command,
    ...
    void AtlasWriter::ProcessSpriteItem(flash::Shape& shape,
                                        wk::AtlasGenerator::Item& atlas_item,
                                        BitmapItem& sprite_item) {
        ProcessVertices(shape, atlas_item.vertices, atlas_item, sprite_item);
    }

    void AtlasWriter::ProcessSlicedItem(flash::Shape& shape,
                                        wk::AtlasGenerator::Item& atlas_item,
                                        SlicedItem& sliced_item) {
        using namespace wk;
        using namespace wk::AtlasGenerator;

        Item::Transformation<float> transform(0, sliced_item.Translation());

        Container<Container<VertexF>> regions;
        atlas_item.get_9slice(sliced_item.Guides(), regions, transform);
    ...
    void AtlasWriter::WriteAtlas() {
        ...
        StatusComponent* status =
            context.Window()->CreateStatusBarComponent(context.locale.GetString("TID_STATUS_SPRITE_PACK"));
        ...
        for (StaticElementsGroup& group : m_writer.m_graphic_groups) {
            for (size_t i = 0; group.Size() > i; i++) {
                ...
                if (item.IsSprite()) {
                    ...
                    auto& atlas_item = items.emplace_back(sprite_item.Image(), item.Is9Sliced());
        ...
        AtlasGenerator::Config generator_config(config.textureMaxWidth,
        ...
        generator_config.progress = [&status](uint32_t value) { status->SetProgress(value); };
        ...
        try {
            ...
            if (exception.reason() == AtlasGenerator::PackagingException::Reason::Unknown) {
                throw SCPluginException(wk::StringConverter::ToUTF16(
                    fmt::format("[AtlasGenerator] {}", context.locale.GetU8String("TID_SWF_ATLAS_UNKNOWN"))));
            }
            ...
            if (exception.index() != SIZE_MAX) {
                for (size_t group_index = 0, atlas_item_index = 0; m_writer.m_graphic_groups.size() >= group_index;
                     group_index++) {
        ...
        FINALIZE_THROW:
            throw SCPluginException(
                wk::StringConverter::ToUTF16(fmt::format("[AtlasGenerator] {} \"{}\"",
                                                         wk::StringConverter::ToUTF8(reason),
                                                         wk::StringConverter::ToUTF8(symbol_name))));
        }
            ...
            for (uint32_t group_item_index = 0; group.Size() > group_item_index; group_item_index++) {
                ...
                if (item.IsSprite()) {
                    if (item.Is9Sliced()) {
                        ...
                        ProcessSlicedItem(shape, atlas_item, sliced_item);
                    } else {
                        BitmapItem& sprite_item = *(BitmapItem*) &item;
                        ProcessSpriteItem(shape, atlas_item, sprite_item);
                    }
                } else if (item.IsSolidColor() && atlas_item.get_colorfill().has_value()) {
                    FilledItem& filled_item = *(FilledItem*) &item;
                    ProcessFilledItem(shape, atlas_item, filled_item);
                }
        ...
        auto encode_texture = [&config](flash::SWFTexture& texture, size_t) {
        ...
        wk::parallel::enumerate(m_writer.swf.textures.begin(), m_writer.swf.textures.end(), encode_texture);
    ...
    void AtlasWriter::ProcessFilledItem(flash::Shape& shape,
                                        wk::AtlasGenerator::Item& atlas_item,
                                        FilledItem& filled_item) {
        if (!atlas_item.get_colorfill().has_value())
            return;
