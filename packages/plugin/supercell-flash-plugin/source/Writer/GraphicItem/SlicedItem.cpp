...
namespace sc {
    namespace Adobe {
        SlicedItem::SlicedItem(Animate::Publisher::SymbolContext& context,
        ...
        wk::RectF SlicedItem::RoundScalingGrid(const Animate::DOM::Utils::RECT& guides) {
            return wk::RectF(std::min(guides.topLeft.x, guides.bottomRight.x),
                             std::max(guides.topLeft.y, guides.bottomRight.y),
                             std::max(guides.topLeft.x, guides.bottomRight.x),
                             std::min(guides.topLeft.y, guides.bottomRight.y));
        }
    }
}