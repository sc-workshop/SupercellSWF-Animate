...
namespace sc::flash {
    ...
    bool is_solid_9slice(const Shape& shape) {
        for (uint32_t i = 0; shape.commands.size() > i; i++) {
            ...
            for (uint32_t t = 0; shape.commands.size() > t; t++) {
                ...
                if (shape.commands[i].texture_index != shape.commands[t].texture_index)
                    return false;
                ...
                for (uint32_t v = 0; first.vertices.size() > v; v++) {
                    ...
                    auto it =
                        std::find_if(second.vertices.begin(), second.vertices.end(), [&vertex](const auto& other) {
    ...
    ShapeDrawBitmapCommand create_proxy_9slice_command(const Shape& shape) {
        ...
        result.vertices.emplace_back(ShapeDrawBitmapCommandVertex {min.x, max.y, min.u, max.v});

        result.vertices.emplace_back(ShapeDrawBitmapCommandVertex {max.x, max.y, max.u, max.v});

        result.vertices.emplace_back(ShapeDrawBitmapCommandVertex {max.x, min.y, max.u, min.v});

        result.vertices.emplace_back(ShapeDrawBitmapCommandVertex {min.x, min.y, min.u, min.v});
    ...
    void repack_atlas(SupercellSWF& swf) {
        ...
        auto get_index = [&bitmaps](const ShapeDrawBitmapCommand& bitmap) -> std::optional<size_t> {
            for (size_t i = 0; bitmaps.size() > i; i++) {
                ...
                if (bitmap.texture_index != other.texture_index)
                    continue;
                if (bitmap.vertices.size() != other.vertices.size())
                    continue;
            ...
            switch (image->base_type()) {
                ...
                case Image::BasePixelType::LA:
                    if (image->depth() != Image::PixelDepth::LUMINANCE8_ALPHA8) {
                        result =
                            CreateRef<RawImage>(image->width(), image->height(), Image::PixelDepth::LUMINANCE8_ALPHA8);
                        RawImage::remap(image->data(),
                                        result->data(),
                                        image->width(),
                                        image->height(),
        ...
        parallel::enumerate(
            bitmaps.begin(),
            bitmaps.end(),
            [&images, &items](const ShapeDrawBitmapCommand& bitmap, size_t n) {
                auto& texture = images[bitmap.texture_index];
                AtlasGenerator::RectUV bitmap_bound {0xFFFF, 0, 0, 0xFFFF};
                ...
                for (const ShapeDrawBitmapCommandVertex& vertex : bitmap.vertices) {
                    AtlasGenerator::PointUV point((uint16_t) std::ceil(vertex.u * texture->width()),
                ...
                auto process_sprite = [&]() {
                    ...
                    if (!success) {
                        throw Exception("Looks like this sc contains non-convex polygons and this is really bad! Atlas "
                                        "repackaging is impossible....");
                    }
                };
        ...
        auto* status = context.Window()->CreateStatusBarComponent(context.locale.GetString("TID_STATUS_SPRITE_PACK"));
        ...
        config.progress = [&status](uint32_t value) { status->SetProgress(value); };
        ...
        for (Shape& shape : swf.shapes) {
            if (nine_scalings_shapes.count(shape.id)) {
                ...
                for (ShapeDrawBitmapCommand& command : shape.commands) {
                    ...
                    for (auto& vertex : command.vertices) {
                        wk::PointF uv_vertex {vertex.u - uv_offset.x, vertex.v - uv_offset.y};
