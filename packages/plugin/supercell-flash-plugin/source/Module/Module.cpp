...
namespace sc::Adobe {
    const Animate::ModuleInfo SCPlugin::SCPluginInfo =
        Animate::ModuleInfo(PLUGIN_VERSION, PUBLISHER_NAME, PUBLISHER_UNIVERSAL_NAME, "", PUBLISH_SETTINGS_UI_ID);

    SCPlugin::SCPlugin(FCM::PIFCMCallback callback) :
        FCM::PluginModule(callback, SCPluginInfo) {
    ...
    void SCPlugin::InitializeLogData() {
        auto application =
            GetService<Animate::Application::Service::IApplicationService>(Animate::Application::Service::APP_SERVICE);
...

        {
            ...
            if (!wheelchair.IsWheelchairMoving()) {
                logger->error("Wheelchair is standing still :sadness_combat:. Some functions will be disabled and will "
                              "not work correctly.");
            }
        }
    }
