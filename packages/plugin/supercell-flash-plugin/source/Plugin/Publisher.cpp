...
template <>
const Animate::FCMPluginID& ::Animate::Publisher::GenericPublisher<SCConfig, SCPublisher>::PluginID() {
    static Animate::FCMPluginID id(CLSID_Publisher, CLSID_DocType, CLSID_FeatureMatrix);
    return id;
}
...
namespace sc::Adobe {
    ...
    void SCPublisher::PublishDocuments() {
        ...
        StatusComponent* publishStatus =
            context.Window()->CreateStatusBarComponent(context.locale.GetString("TID_STATUS_INIT"));
        ...
        auto publish = [&](Animate::DOM::PIFLADocument document) {
            fs::path document_path = context.falloc->GetString16(document, &Animate::DOM::IFLADocument::GetPath);

            publishStatus->SetStatusLabel(
                context.locale.GetString("TID_WINDOW_TITLE", document_path.filename().string()));
        ...
        publishStatus->SetStatus(context.locale.GetString("TID_STATUS_SAVE"));
    ...
    void SCPublisher::LoadDocument(const fs::path& path, Animate::DOM::PIFLADocument& document) {
        ...
        FCM::Result status =
            service->OpenDocument((FCM::CStringRep16) FCM::Locale::ToUtf16(path.string()).c_str(), document);
    ...
    void SCPublisher::Publish(const SCConfig& /*config*/) {
        ...
        context.Trace(context.locale.GetString("TID_EXPORT_TIME_STATUS", executionTime));
    }
    ...
    void SCPublisher::DoPublish() {
        ...
        if (config.useMultiDocument) {
            StatusComponent* documentProgress =
                context.Window()->CreateStatusBarComponent(context.locale.GetString("TID_LABEL_DOCUMENT_LOADING"));
