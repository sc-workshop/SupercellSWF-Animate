...
export default function TextureSettings() {
	const [textureEncodingMethod, setTextureEncodingMethod] =
		useState<TextureEncoding>(Settings.getParam("textureEncoding"));
	const [useMultiresTexture, setUseMultiresTexture] = useState<boolean>(
		Settings.getParam("hasMultiresTexture"),
	);
	const [useLowresTexture, setUseLowresTexture] = useState<boolean>(
	...
	const textureCompressExternalFile = new BoolField({
		...
		style: defaultStyle,
		callback: (value) =>
			Settings.setParam("compressExternalTextureFile", value),
	...
	const textureQuality = new EnumField({
		...
		style: defaultStyle,
		callback: (value) =>
			Settings.setParam("textureQuality", parseInt(value, 10)),
	});
	...
	const textureWidth = new EnumField({
		...
		defaultValue: TextureDimensions.indexOf(
			Settings.getParam("textureMaxWidth"),
		) as never,
		style: {
			display: "flex",
			alignItems: "center",
	...
	const textureHeight = new EnumField({
		...
		enumeration: TextureDimensions,
		defaultValue: TextureDimensions.indexOf(
			Settings.getParam("textureMaxHeight"),
		) as never,
		style: {
			display: "flex",
			alignItems: "center",
	...
	const multiresProps = renderComponents(
		[multiresSuffix, lowresSuffix],
		useMultiresTexture,
	);
	const lowresProps = renderComponents(
		[generateLowresTextures],
		useLowresTexture,
	);
	...
	const generalCompressedTextureProps = renderComponents(
	...
	const supercellTextureProps = renderComponents(
	...
	const advancedVersionDependentProps = renderComponents(
		[exportToExternal],
